...
def bin_path_to_dir(bin_path: str) -> str:
    ...
    bin_bash_name = os.path.basename(bin_path)
    if bin_path.startswith('http'):
        assert bin_path.endswith('.zip')  # for now only support zip from url
        new_bin_path = os.path.join(_tmp_dir(), f'{bin_hash}', bin_bash_name)
    ...
    if bin_path.endswith('.zip'):
        new_bin_path = os.path.join(_tmp_dir(), f'{bin_hash}', bin_bash_name.removesuffix('.zip'))
