...
from esptest.jira.client import DEFAULT_TIMEOUT
...
def parse_args(argv: t.Optional[t.List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f'Jira request timeout in seconds (default: {DEFAULT_TIMEOUT}).',
    )
...
def _client_from_args(args: argparse.Namespace) -> t.Any:
    if args.server or args.token or args.timeout != DEFAULT_TIMEOUT:
        ...
        if args.name or args.attachment_id:
            ...
            if args.attachment_id:
                ...
                if attachment is None:
                    raise FileNotFoundError(f'Jira attachment {args.attachment_id} was not found')
            ...
            downloaded_files = download_all_attachments(client, args.issue, destination_dir)
            logging.info('Downloaded %d attachment(s) to %s.', len(downloaded_files), destination_dir)
    else:
        for attachment in list_attachments(client, args.issue):
            print(f'{attachment.id}\t{attachment.size}\t{attachment.filename}')
