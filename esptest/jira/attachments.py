...
def _zip_directory(source: Path) -> Path:
    ...
    archive = temp_dir / f'{source.name}.zip'
    try:
        file_count = 0
...
def _find_attachment(
    ...
    raise FileNotFoundError(f'Jira attachment {target} was not found')
...
def _unique_destination(directory: Path, filename: str) -> Path:
    ...
    while candidate.exists():
        candidate = directory / f'{Path(safe_filename).stem} ({index}){Path(safe_filename).suffix}'
        index += 1
    return candidate
