...
def test_main_uploads_paths() -> None:
    ...
    with mock.patch.object(jira_attachment, 'parse_args', return_value=args), mock.patch.object(
        jira_attachment, 'login_jira', return_value=client
    ), mock.patch.object(jira_attachment, 'upload_attachments', return_value=[object()]) as upload:
        jira_attachment.main()

    upload.assert_called_once_with(client, 'TEST-1', ['log.txt'], filename=None)


def test_main_downloads_all_to_destination(tmp_path) -> None:
    args = SimpleNamespace(
        command='download',
        issue='TEST-1',
        name=None,
    ...
    with mock.patch.object(jira_attachment, 'parse_args', return_value=args), mock.patch.object(
        jira_attachment, 'login_jira', return_value=client
    ), mock.patch.object(jira_attachment, 'download_all_attachments', return_value=['a', 'b']) as download:
        jira_attachment.main()

    download.assert_called_once_with(client, 'TEST-1', str(tmp_path))


def test_main_downloads_attachment_id_with_original_filename(tmp_path) -> None:
    args = SimpleNamespace(
        command='download',
        issue='TEST-1',
        name=None,
    ...
    ), mock.patch.object(jira_attachment, 'download_attachment', return_value='output/log.txt') as download:
        jira_attachment.main()
...
def test_main_lists_attachments(capsys) -> None:
    args = SimpleNamespace(
        command='list',
        issue='TEST-1',
        server=None,
