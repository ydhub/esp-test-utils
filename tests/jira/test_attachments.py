...
class FakeClient:
    def __init__(self, attachments: t.Optional[t.List[FakeAttachment]] = None) -> None:
        self.attachments = attachments or []
        self.uploads: t.List[t.Dict[str, t.Any]] = []

    def issue(self, issue_key: str) -> SimpleNamespace:
        assert issue_key == 'TEST-1'
        return SimpleNamespace(fields=SimpleNamespace(attachment=self.attachments))

    def add_attachment(self, **kwargs: t.Any) -> t.Dict[str, t.Any]:
...
def test_upload_file_forwards_custom_filename(tmp_path: Path) -> None:
...
def test_upload_directory_zips_and_cleans_temporary_archive(tmp_path: Path) -> None:
...
def test_download_attachment_by_id(tmp_path: Path) -> None:
...
def test_download_all_preserves_duplicate_filenames(tmp_path: Path) -> None:
    client = FakeClient(
        [
    ...
    assert [Path(item).name for item in downloaded] == ['log.txt', 'log (1).txt', 'unsafe.txt']
...
def test_upload_rejects_custom_name_for_multiple_files(tmp_path: Path) -> None:
